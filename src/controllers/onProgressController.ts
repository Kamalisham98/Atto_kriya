...
class OnProgressController {
  async get(req: Request, res: Response) {
    try {
      const response: QueryResult = await pool.query(
        `SELECT * FROM progressboard where onprogress = true`
      );
  ...
  async add(req: Request, res: Response) {
    try {
      ...
      await pool.query({
        text: "INSERT INTO progressboard (activity,todo,onprogress,done) VALUES($1,false,true,false)",
        values: [activity],
      });
  ...
  async updateToDone(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const response: QueryResult = await pool.query({
        text: "UPDATE progressboard SET todo = false, onprogress = false, done = true where id = $1",
        values: [id],
      });
      const find: QueryResult = await pool.query({
        text: "SELECT * from progressboard where id = $1",
...
export default new OnProgressController();
